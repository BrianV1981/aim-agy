...
2. **Agentic Reincarnation Protocol:** When the Operator types `/reincarnate` or `/python3 .aim_core/aim_cli.py reincarnate`, you MUST manually execute the handoff. Use the `run_shell_command` tool to:
   a. Write a highly structured handover message to `continuity/REINCARNATION_GAMEPLAN.md` explicitly following the 5-section format mandated in `aim-agy_os/GAMEPLAN_SOP.md` (Commander's Summary, Tactical State, Localized Directory Map, Epistemic Warnings, and Immediate Next Action).
