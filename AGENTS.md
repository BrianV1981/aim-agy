...
3. **Worktree Hygiene:** A.I.M. creates isolated Git Worktrees in the `workspace/` directory for each issue (`python3 .aim_core/aim_cli.py fix <id>`). To prevent the Antigravity CLI from recursively scanning hundreds of redundant files across multiple branches, you MUST ensure that `workspace/` is listed in your `.agyignore` file. When an issue is complete, actively clean up the worktree using `python3 .aim_core/aim_cli.py promote` or `git worktree remove` to prevent context bloat.
